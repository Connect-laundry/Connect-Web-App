...
export const OrderModalFooter = ({ orderId, onClose, availableActions, isLoading, onAction }: OrderModalFooterProps) => {
    const showsDeliveryReleaseNote = availableActions.includes("markDelivered") || availableActions.includes("complete");

    return (
        <div className="border-t border-border/40 bg-muted/20">
            {showsDeliveryReleaseNote && (
                <div className="px-6 pt-4 text-xs leading-relaxed text-muted-foreground">
                    Your earnings are released after the customer confirms receiving the order, or after the protection period if no issue is reported.
                </div>
            )}
            <DialogFooter className="px-6 py-4 flex flex-col sm:flex-row gap-2 justify-between items-center">
